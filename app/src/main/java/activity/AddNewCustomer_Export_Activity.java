...
public class AddNewCustomer_Export_Activity extends AppCompatActivity {
    Intent intent = null;
    ...
    android.os.Handler mHandler = new android.os.Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Toast.makeText(AddNewCustomer_Export_Activity.this, mString, Toast.LENGTH_LONG).show();
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_new_customer_export);
        context = this;
        AddNewCustomer_Export_Activity.this.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
...
//                        validateTluka()         &&
    ...
    public boolean validateMarketplace() {
        if (edittext_market.getText().toString().trim().isEmpty()) {
            input_market.setError("Please Enter Market Place");
            requestFocus(edittext_market);
            return false;
        } else {
            input_market.setErrorEnabled(false);
        }
        return true;
    }
    ...
    public void getAreaDistributor() {
        ...
        try {
            ...
            if (vkorg.equals("1200") && vtweg.equals("11")) {
                ...
                selectQuery = "SELECT * FROM " + DatabaseHelper.TABLE_AREA_DISTRIBUTOR
                        + " WHERE " + DatabaseHelper.KEY_LAND_TXT + " = '" + country + "'"
                        + " AND " + DatabaseHelper.KEY_STATE_TXT + " = '" + state + "'";
            }
            Cursor cursor = db.rawQuery(selectQuery, null);
            Log.d("bean1", "" + cursor.getCount() + "--" + vkorg + "--" + country + "--" + state);
            if (cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
...
//                district,//jo.getString("district_txt"),
                // pass market place to district text
                edittext_market.getText().toString().toLowerCase(),//jo.getString("district_txt"),
                ...
                edittext_landline.getText().toString(), // tel phone no
                edittext_aadharcard.getText().toString(), // aadhar card
                edittext_pancard.getText().toString(), //pan card
                edittext_tinno.getText().toString(), // tin no
                edittext_market.getText().toString().toLowerCase(), // market
                "",//edittext_dob.getText().toString().toLowerCase().trim(), // dob

                edittext_pincode.getText().toString(),

                edittext_contact_person_name.getText().toString().toLowerCase(),
                edittext_contact_person_phone.getText().toString(),
    ...
    public void SyncNewAdddedCustomerInBackground() {
        Intent i = new Intent(AddNewCustomer_Export_Activity.this, SyncDataService.class);
        startService(i);
    }
