...
import adapter.PendingComplainDetailsListAdapter;
...
private String textRemarkValue;
    ...
    private void initView() {
        ...
        mRemarkResponseList = new ArrayList<>();
    ...
    public void callgetCompalinAllListAPI() {
        ...
        param.clear();
      //  param.add(new BasicNameValuePair("cmpno", "EU2733"));//kunur it means user id
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    String obj = CustomHttpClient.executeHttpPost1(WebURL.PENDING_COMPLAIN_ALL_REMARK_VK_PAGE, param);
                    ...
                    if (mStatus.equalsIgnoreCase("true")) {
                        ...
                        if(mRemarkResponseList.size()>0)
                            mRemarkResponseList.clear();
                        ...
                        for (int i = 0; i < ja.length(); i++) {
                            ...
                            RemarkResponseList mmRemarkResponseList = new RemarkResponseList();

                            mmRemarkResponseList.setCmpno(join.getString("cmpno"));
                            mmRemarkResponseList.setCounter(join.getString("counter"));
                            mmRemarkResponseList.setPernr(join.getString("pernr"));
                            mmRemarkResponseList.setEname(join.getString("ename"));
                            mmRemarkResponseList.setAedtm(join.getString("aedtm"));
                            mmRemarkResponseList.setAction(join.getString("action"));
                            mmRemarkResponseList.setStatus(join.getString("status"));
                            mmRemarkResponseList.setCmpPenRe(join.getString("cmp_pen_re"));
                            mmRemarkResponseList.setSrvKunnr(join.getString("srv_kunnr"));


                            mRemarkResponseList.add(mmRemarkResponseList);
                        ...
                        runOnUiThread(new Runnable() {
                            public void run() {
                                ...
                                mPendingComplainRemarkListAdapter = new PendingComplainRemarkListAdapter(mContext, mRemarkResponseList, mStatusValue, mMobileNumber);
                                rclyPendingComplainList.setAdapter(mPendingComplainRemarkListAdapter);
                                progressDialog.dismiss();
                            }
